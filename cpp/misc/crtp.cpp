...
class ChildFirst : public Parent<ChildFirst> {
 public:
  void talkImpl() { std::cout << "in crtp ChildFirst talk impl" << std::endl; }
};

class ChildSecond : public Parent<ChildSecond> {
 public:
  void talkImpl() { std::cout << "in crtp ChildSecond talk impl" << std::endl; }
};

int main(int argc, char *argv[]) {
  std::shared_ptr<ChildFirst> ptr1 = std::make_shared<ChildFirst>(ChildFirst());
